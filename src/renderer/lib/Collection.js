...
class Collection {
  ...
  groupBy(property) {
    ...
    this.each((item) => {
      let index = isFunction ? property(item) : item[property];
  ...
  offset(amount) {
    ...
    return new Collection(this.data.slice(amount, this.data.length));
  }
  ...
  sum(property = null) {
    ...
    return this.reduce((carry, item) => {
      return !item[property] === ""
        ? carry
        : carry + parseFloat(item[property]);
    });
  }
